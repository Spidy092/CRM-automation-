...
const actionParameters: Record<AgentActionName, Record<string, unknown>> = {
  'lead.list': {
    type: 'object',
    properties: { limit: { type: 'number' }, search: { type: 'string' }, classification: { type: 'string' }, status: { type: 'string' } },
  },
  'lead.get': { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
  'lead.create': { type: 'object', additionalProperties: true },
  'lead.update': { type: 'object', properties: { id: { type: 'string' }, input: { type: 'object' } }, required: ['id', 'input'] },
  'lead.pause': { type: 'object', properties: { id: { type: 'string' }, paused: { type: 'boolean' } }, required: ['id', 'paused'] },
  'pipeline.move_lead': { type: 'object', properties: { leadId: { type: 'string' }, stageId: { type: 'string' } }, required: ['leadId', 'stageId'] },
  ...
  'assignment.override': { type: 'object', properties: { leadId: { type: 'string' }, newUserId: { type: 'string' }, reason: { type: 'string' } }, required: ['leadId', 'newUserId', 'reason'] },
  'report.dashboard': { type: 'object', properties: {} },
  'scraper.run': { type: 'object', properties: { configId: { type: 'string' } }, required: ['configId'] },
  'outreach.send_manual': { type: 'object', additionalProperties: true },
  'ai.decision.recompute': { type: 'object', properties: { leadId: { type: 'string' }, force: { type: 'boolean' }, context: { type: 'object' } }, required: ['leadId'] },
  'ai.inbox.action': {
    type: 'object',
    properties: {
      id: { type: 'string' },

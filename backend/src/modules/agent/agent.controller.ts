...
export async function proposeAction(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    res.status(result.policy.outcome === 'require_approval' ? 202 : 200).json(successResponse(result));
  } catch (err) {
    next(err);
  }
}

export async function executeAction(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const actor = toAgentActor(req.user!, req.ip);
    const action = await executeAgentAction(req.params.id, { actor, approvedBy: actor.id, source: 'manual' });
